...
for i in $(seq 0 "$((${shards}-1))"); do
    ...
    for j in {0..2}; do
        ...
        python sisa.py --model purchase --train --slices 32 --dataset datasets/purchase/datasetfile --label "${r}" --epochs 1 --batch_size 16 --learning_rate 0.001 --optimizer sgd --chkpt_interval 1 --container "${shards}" --shard "${i}"
    done
done
