...
python distribution.py --shards "${shards}" --distribution uniform --container "${shards}" --dataset datasets/sst2/datasetfile --label 0

if ((ul_reqs >= 1)); then
    for j in $(seq 1 $((ul_reqs))); do
        r=$((${j}))
        echo "${r}"
        python distribution.py --requests "${r}" --distribution uniform --container "${shards}" --dataset datasets/sst2/datasetfile --label "${r}"
    done
fi
